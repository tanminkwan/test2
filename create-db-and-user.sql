...
-- 실행 방법: psql -U postgres -h localhost -f create-db-and-user.sql

-- 1. user_service 데이터베이스 생성 (이미 있으면 무시)
...
-- 4. user_service 데이터베이스에 대한 권한 부여
GRANT ALL PRIVILEGES ON DATABASE user_service TO app_user;

-- 5. user_service 데이터베이스에 연결하여 스키마 권한 설정
\c user_service
