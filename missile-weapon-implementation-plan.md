...
### 단계 1: 기본 설정 및 구조 구축 (완료)
**작업:**
- [x] `game-config.yaml`에 미사일 기본 설정 추가
- [x] 빈 `Missile.js` 및 `GuidedMissile.js` 파일 생성 (기본 클래스만 구현)
- [x] `WeaponSystem.js`에 미사일 무기 타입 등록 (실제 기능 없이)

**테스트:**
- [x] 설정 파일이 올바르게 로드되는지 확인 (로그 출력)
- [x] 게임이 crash 없이 시작되는지 확인

### 단계 2: 미사일 발사 및 클라이언트 렌더링 (완료)
**작업:**
- [x] `Missile.js`에 기본 물리 동작 구현 (직선 운동)
- [x] `GuidedMissile.js`에서 미사일 생성 로직 구현
- [x] 클라이언트에 미사일 3D 모델 추가 및 렌더링 연결
- [x] `GameManager.js`에 미사일 발사 입력 처리 추가

**테스트:**
- [x] 키보드('M')로 미사일 발사 시도
- [x] 미사일이 화면에 제대로 렌더링되며 직선으로 발사되는지 확인
- [x] 미사일 제한 수량이 UI에 표시되는지 확인

### 단계 3: 미사일 유도 기능 구현 (서버) (완료)
**작업:**
- [x] `GameManager.js`에 가장 가까운 적 자동 타겟팅 로직 추가
- [x] `Missile.js`에 타겟 추적 알고리즘 추가
- [x] `WeaponSystem.js`에서 타겟 정보를 `Missile` 객체에 전달하도록 수정

**테스트:**
- [x] 발사된 미사일이 타겟을 향해 방향을 전환하는지 확인 (서버 로그 및 클라이언트 움직임)
- [x] 타겟이 회피 기동해도 계속 추적하는지 확인
- [x] 타겟이 없거나 범위를 벗어나면 직진하는지 확인

### 단계 4: 타겟팅 시스템 및 UI 구현
**작업:**
- HUD에 현재 타겟 정보(거리 등) 및 타겟팅 UI(박스, 표시기) 추가
- 타겟 락온 시각적/청각적 효과 구현
- 수동으로 타겟을 변경하는 기능 추가 (예: 'T' 키)

**테스트:**
- 타겟팅된 적 주변에 UI 요소가 표시되는지 확인
- 락온 시 UI가 변경되고 효과가 나타나는지 확인
- 타겟 변경 키를 눌렀을 때 다른 적으로 타겟이 바뀌는지 확인

### 단계 5: 충돌 및 효과음, 시각 효과 개선
**작업:**
- 미사일 충돌 시 폭발 효과 및 데미지 적용
- 미사일 연기(Trail) 효과 추가
- 미사일 발사, 비행, 폭발 관련 효과음 추가
- 범위 데미지 기능 구현 (설정에 따라)

**테스트:**
- 미사일이 적과 충돌 시 폭발하는지 확인
- 폭발 효과(시각/음향)가 재생되는지 확인
- 미사일 비행 시 연기 효과가 나타나는지 확인

### 단계 6: 리팩토링 및 밸런스 조정
**작업:**
- 코드 정리 및 주석 추가
- `game-config.yaml` 값을 조정하여 게임 밸런스 맞추기 (미사일 속도, 데미지, 추적 성능 등)
- 플레이 테스트를 통한 최종 검증

**테스트:**
- 다수 플레이어 환경에서 성능 저하 없는지 확인
- 미사일 무기가 게임 밸런스를 해치지 않는지 확인
